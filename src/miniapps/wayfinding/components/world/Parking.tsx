...
import { PARKINGS } from "@/miniapps/wayfinding/lib/layout";
      ...
      {PARKINGS.map((p, i) => {
        ...
        return (
          <group key={i} position={[x, 0, z]} rotation={[0, rot, 0]}>
            ...
            {cars}
            <Html position={[0, 3, -d / 2 - 1]} center distanceFactor={72} zIndexRange={[8, 0]}>
              <div
                className="mono select-none rounded-md px-1.5 py-0.5 text-[10px] font-semibold"
                style={{ background: "rgba(255,255,255,0.8)", color: "#5b6675" }}
              >
                {p.label}
              </div>
            </Html>
